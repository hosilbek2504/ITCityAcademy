...
import it.city.itcityacademy.entity.*;
...
import it.city.itcityacademy.repository.*;
...
public class GroupService {
    ...
    public void addOrEditGroup(ReqGroup reqGroup, Group group) {
        ...
        switch (reqGroup.getWeekday().name()) {
            case "ODD":
                week.add("Monday");
                week.add("Wednesday");
                week.add("Friday");
    ...
    public ResGroup getGroup(Integer id) {
        Group group = groupRepository.findById(id).orElseThrow(() -> new ResourceAccessException("get Group"));
        return new ResGroup(group.getId(), group.getName(), group.getCategory(), group.getStartHour(), group.getFinishHour(), group.getLessonStartedDate(), group.getLessonFinishDate(), group.getTeacher(), group.getWeekday().name());
    }
    ...
    public ApiResponse getGroupPage(Integer page, Integer size) {
        ...
        return new ApiResponse(true, new ResPageable(page, size, groupPage.getTotalPages(), groupPage.getTotalElements(), groupPage.getContent().stream().map(group -> getGroup(group.getId())).collect(Collectors.toList())
